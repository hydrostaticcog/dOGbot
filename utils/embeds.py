...
def leaderboard_embed(ctx: MyContext = None, bot: dOGbot = None, board: List = None):
...
def mod_message_embed(ctx: MyContext = None, bot: dOGbot = None, case_id=0, type="not specified",
                            reason="not given"):
    embed = discord.Embed(title="New Action on your Account", description=f"Type: {type}\nGuild: {ctx.guild.name}\nCase ID: {case_id}",
                          color=bot.color)
